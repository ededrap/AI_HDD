...
class HeartForm(forms.Form):
    ...
    fbs = forms.ChoiceField(label='Fasting Blood Sugar > 120mg/dl?', choices=((1,"Yes"), (0,"No")), widget=forms.Select(attrs=attrs))
    ...
    exang = forms.ChoiceField(label = 'Exercise Induced Angina', choices=((1,"Yes"), (0,"No")), 
        widget=forms.Select(attrs=attrs))
    attrs.update({'placeholder' : 'Oldpeak'})
    ...
    ca = forms.ChoiceField(label = 'Jumlah pembuluh major yang berwarna pada floroskopi', 
        choices=((0,"0"), (1,"1"), (2, "2"), (3, "3")), widget=forms.Select(attrs=attrs))
